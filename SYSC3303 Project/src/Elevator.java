...
public class Elevator extends Thread {
	...
	// public static List<byte[]> ElevatorTable = Collections.synchronizedList(new
	// ArrayList<byte[]>());
	...
	public int runElevator(byte motorDirection) {
		if (motorDirection == UP || motorDirection == DOWN) {
			try {
				System.out.println("current floor: " + sensor + " --> of Elevator " + nameOfElevator); // sensor =
																										// current floor
				Thread.sleep(2000);
		...
		System.out.println("current floor: " + sensor + " --> of Elevator " + nameOfElevator); // prints out the current
																								// floor - in this case
																								// destination floor
		return getInitialFloor(); // returns and updates the final current of the floor - in this case destination
									// floor
	}
	...
	public synchronized void sendPacket(byte[] toSend) throws InterruptedException {
		...
		try {
			...
			// System.out.println("\nSending to scheduler from Elevator "+ data[1] + ":" +
			...
			while (runningStatus == true) {
				if (toDoID == nameOfElevator) {
					...
					if (instruction == 2 || instruction == 1) {
						...
						/*
						 * try { Thread.sleep(1); sendPacket(responsePacketRequest(UPDATE,0)); } catch
	...
	public static void main(String args[]) throws InterruptedException {
		 ...
		 * 
		 * sendPacket(ElevatorTable1.get(0)); sendPacket(ElevatorTable1.get(1));
		...
		try {
			while (true) {
				...
				if (x[1] == 3) {
					...
					System.out.println(
							"        ELEVATOR 4 INSTRUCTION               " + data[6] + " ELEVATOR ID " + data[1]);
