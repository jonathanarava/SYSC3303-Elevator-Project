...
/*		requestElevator = elevatorArray[0].responsePacketRequest(1);// elevatorArray[0].floorRequest);
		...
		synchronized (elevatorTable) {
			...
			try {
				System.out.println("\nSending to scheduler from Elevator "+ data[1] + ":" + Arrays.toString(elevatorTable.get(0)));
	...
	public synchronized void receivePacket() throws InterruptedException {
		...
		if (data[1] == 0) {
			...
			//elevatorArray[1].runElevator(data[6]);
		}
	...
	public static void main(String args[]) throws IOException, InterruptedException {// 2 arguments: args[0] is the
		...
		for (int i = 0; i < createNumElevators; i++) {
			 ...
			 * system is passed via elevatorArray[i] = new Elevator(i, floorRequest);
			 * floorRequest = Integer.parseInt(args[i+2]);// The number of Elevators in the
			 * system is passed via elevatorArray[i] = new Elevator(i, floorRequest);
			 */// i names the elevator, 0 initializes the floor it starts on
