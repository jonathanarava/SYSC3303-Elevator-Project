...
//					}
					...
					/*else if (elevatorState == UPDATE_DISPLAY) {//else if (motorDirection == UPDATE_DISPLAY) {
						...
						System.out.println("Reached Hold state in elevator: "+elevatorNumber);
						dealWith =false;// !dealWith;
						//sendPacket(UPDATE,UNUSED, NO_ERROR);
						//waitForRequest();
					}
						...
						System.out.println("Elevator: "+elevatorNumber+" SHUT_DOWN");
						shutDown();
						dealWith=false;//
						//sendPacket(UPDATE,UNUSED, NO_ERROR);
					}
...
//					}
	...
	public void runElevator() {//int direction) {
		...
		else if (elevatorState==STOP||elevatorState==HOLD) {
			System.out.println("runElevator(); stop or hold");
		}
		else {
